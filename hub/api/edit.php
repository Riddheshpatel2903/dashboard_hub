...
$title = $input['title'] ?? 'Updated Title'; // YouTube title edit support

if ($postId <= 0 || empty($newContent)) {
    header('Content-Type: application/json', true, 400);
    echo json_encode(['success' => false, 'error' => 'Missing post_id or content parameter']);
    exit();
}
...
try {
    ...
    // 3. Dispatch Edit request if external ID is present
    if (!empty($externalPostId)) {
        switch ($platform) {
            ...
            case 'linkedin':
                $response = LinkedInHandler::editPost($token, $externalPostId, $newContent);
                break;
                
            case 'google_business':
                // GBP Local Posts update via PATCH
                $response = GoogleBusinessHandler::updateBusinessInfo($token, $externalPostId, [
                    'summary' => $newContent
                ]);
                break;
    
            case 'youtube':
                $response = YouTubeHandler::editVideoMetadata($token, $externalPostId, $title, $newContent);
                break;
    
            default:
                throw new Exception("Editing is not supported for {$platform} posts.");
        }
    } else {
        $response = ['message' => 'Post content updated locally (no external post ID found).'];
    }

    // 4. Update the DB on success
    $stmt = $pdo->prepare("
        UPDATE posts 
        SET content = :content 
        WHERE id = :post_id
    ...
    log_message('error', "Failed to edit post ID {$postId} on {$platform}", ['error' => $e->getMessage()]);
